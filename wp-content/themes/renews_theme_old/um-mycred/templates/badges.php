...
$mycred_options = mycred_get_option( 'mycred_pref_core' ); ?>
			...
			if ( ! empty( $base_requirements ) ) {
				foreach ( $base_requirements as $requirement ) {
					if ( ( $badge_level_reached !== false && $level_key <= $badge_level_reached ) ) { // Unlocked
						$badge_title .= sprintf( _x( '&#013;&#10;%s %s for "%s"', '"Gained/Lost" "x %s" for "reference"', 'um-mycred' ), ( ( $requirement['amount'] < 0 ) ? __( 'Lost', 'um-mycred' ) : __( 'Gained', 'um-mycred' ) ), $mycred->format_creds( $requirement['amount'] ), $requirement['ref'], lcfirst( $mycred_options['name']['plural'] ) );
					} else { // Locked
						$badge_title .= sprintf( _x( '&#013;&#10;%s %s for "%s"', '"Gain/Lost" "x %s" for "reference"', 'um-mycred' ), ( ( $requirement['amount'] < 0 ) ? __( 'Lost', 'um-mycred' ) : __( 'Gain', 'um-mycred' ) ), $mycred->format_creds( $requirement['amount'] ), $requirement['ref'], lcfirst( $mycred_options['name']['plural'] ) );
					}
				}
			}
