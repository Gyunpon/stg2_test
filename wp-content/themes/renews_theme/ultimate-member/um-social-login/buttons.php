...
<div class="um um-shortcode-social" id="um-shortcode-social-<?php echo esc_attr( $id ); ?>" style="padding:<?php echo esc_attr( $padding ); ?>;margin:<?php echo esc_attr( $margin ); ?>!important;">
	...
	<div class="um-field snsLoginBtnListWrap">
		...
		<div class="um-col-alt snsLoginBtnList">
			...
			<div <?php if ( $button_style == 'floated' ) { echo 'style="display:inline"'; } ?> class="<?php if ( $button_style == 'responsive' ) echo esc_attr( $class ) . ' um-half'; ?> snsLoginBtn">
					<a href="<?php echo esc_url( UM()->Social_Login_API()->hybridauth()->getConnectUrl( $provider, $id ) ); ?>" title="<?php echo esc_attr( $arr['button'] ); ?>"
					   class="hoverOpacity um-button um-alt um-button-social um-button-<?php echo $provider; ?>" data-redirect-url="<?php echo esc_url( UM()->Social_Login_API()->hybridauth()->getCurrentUrl() ); ?>"  onclick="um_social_login_oauth_window( this.href,'authWindow', 'width=600,height=600,scrollbars=yes' );return false;">
					<?php if ( $show_icons ) { ?>
