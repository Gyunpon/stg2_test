...
class Social_Login_Connect{
	...
	function has_account_linked( $provider, $userProfile ){
		...
		$automatic_login_email_exists = apply_filters('um_sso_automatic_login_with_email', false );
		if( $automatic_login_email_exists  && ! $user && isset( $userProfile->email ) && ! empty( $userProfile->email  ) ){
			$user = $wpdb->get_row( $wpdb->prepare(
